...
// 신택스 트리의 루트 노드부터 신택스 트리 및 관련 심볼 테이블과 타입테이블 분석
// 원시 프로그램에서 선언된 전역변수의 크기 계산
...
A_TYPE *sem_expression(A_NODE *node) {
	...
	switch(node->name) {
		...
		case N_EXP_FLOAT_CONST :
			lit.type=float_type; 
			lit.value.f = (float)atof(node->clink); 
...
// 심볼 테이블 목록에 있는 명칭들을 차례로 모두 분석
// 선언문에 나타난 각 지역 변수의 주소 설정 및 크기 계산하여 값 리턴
int sem_declaration_list(A_ID *id, int addr)
{
	int i=addr; 
	while (id) {
...
A_LITERAL getTypeAndValueOfExpression(A_NODE *node) {
	...
	switch(node->name) {
		...
		case N_EXP_FLOAT_CONST : 
			result.type=float_type; 
			result.value.f=(float)atof(node->clink); 
			break;
		case N_EXP_STRING_LITERAL : 
		case N_EXP_ARRAY : 
		case N_EXP_FUNCTION_CALL : 
